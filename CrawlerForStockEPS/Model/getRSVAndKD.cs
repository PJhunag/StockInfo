    ...
    public class getRSVAndKD {
        ...
        public getRSVAndKD (MySqlConnection dbconn, string src) {
            ...
            foreach (KD newKD in KDList) {
                ...
                getTwseKD (newKD, 9, "twse010", "twse011", "twse012");
            ...
            foreach (KD newKD in KDList) {
                ...
                //日KD(range=9,target=twse010,twse011,twse012)
                getTpexKD (newKD, 9, "tpex010", "tpex011", "tpex012");
        ...
        public static List<KD> getTwseList () {
            List<KD> KDList = new List<KD> ();

            // 先取出需要處理的清單
            try {

                string StrSQL = "select twse001,twse002 from twse_t where twse010 is null order by twse001,twse002 ";
            ...
            Console.WriteLine ("CNT:" + KDList.Count);

            return KDList;
        }

        public static void getTwseKD (KD newKD, int range, string RSV, string K, string D) {
        ...
        public static List<KD> getTpexList () {
            List<KD> KDList = new List<KD> ();

            // 先取出需要處理的清單
            try {

                string StrSQL = "select tpex001,tpex002 from tpex_t where tpex010 is null order by tpex001,tpex002 ";
                MySqlCommand myCmd = new MySqlCommand (StrSQL, conn);
                MySqlDataReader reader = myCmd.ExecuteReader (); //execure the reader
                while (reader.Read ()) {
                    KD newKD = new KD ();
        ...
        public static void getTpexKD (KD newKD, int range, string RSV, string K, string D) {
            ...
            //收盤價
            try {
                string StrSQL = "select tpex007 from tpex_t where tpex001 = ?tpex001 and tpex002 = ?tpex002 and tpex007 is not null ";
                MySqlCommand myCmd = new MySqlCommand (StrSQL, conn);
                /*                 Console.WriteLine ("StrSQL:" + StrSQL);
                                Console.WriteLine ("tpex001:" + newKD.no);
                                Console.WriteLine ("tpex002:" + newKD.dt.ToString ("yyyy/MM/dd")); */
                myCmd.Parameters.AddWithValue ("@tpex001", newKD.no);
                myCmd.Parameters.AddWithValue ("@tpex002", newKD.dt.ToString ("yyyy/MM/dd"));
                MySqlDataReader reader = myCmd.ExecuteReader (); //execure the reader
            ...
            try {
                string StrSQL = "select max(p) from ( " +
                    " select tpex005 p from tpex_t where tpex001 = ?tpex001 and tpex002 <= ?tpex002 order by tpex002 desc limit " + range + " ) tmp";
                MySqlCommand myCmd = new MySqlCommand (StrSQL, conn);
                /*                 Console.WriteLine (StrSQL);
                                Console.WriteLine (newKD.no);
                                Console.WriteLine (eTime); */
                myCmd.Parameters.AddWithValue ("@tpex001", newKD.no);
                myCmd.Parameters.AddWithValue ("@tpex002", eTime);
                MySqlDataReader reader = myCmd.ExecuteReader (); //execure the reader
            ...
            try {
                string StrSQL = "select min(p) from ( " +
                    " select tpex006 p from tpex_t where tpex001 = ?tpex001 and tpex002 <= ?tpex002 order by tpex002 desc limit " + range + " ) tmp";
                MySqlCommand myCmd = new MySqlCommand (StrSQL, conn);
                myCmd.Parameters.AddWithValue ("@tpex001", newKD.no);
                myCmd.Parameters.AddWithValue ("@tpex002", eTime);
                MySqlDataReader reader = myCmd.ExecuteReader (); //execure the reader
            ...
            //前一天K,D
            try {
                string StrSQL = "select " + K + "," + D + " from tpex_t where tpex001 = ?tpex001 and tpex002 < ?tpex002e order by tpex002 desc limit 1";
                MySqlCommand myCmd = new MySqlCommand (StrSQL, conn);
                myCmd.Parameters.AddWithValue ("@tpex001", newKD.no);
            ...
            try {
                ...
                string StrSQL = "update tpex_t set " + RSV + "=?RSV, " + K + "=?K, " + D + "=?D where tpex001 = ?tpex001 and tpex002 = ?tpex002 ";
                /*                 Console.WriteLine (StrSQL);
                                Console.WriteLine ("tpex001:" + newKD.no);
                                Console.WriteLine ("tpex002:" + newKD.dt);
                                Console.WriteLine ("newKD.RSV:" + newKD.RSV);
                                Console.WriteLine ("newKD.K:" + newKD.K);
                                Console.WriteLine ("newKD.D:" + newKD.D); */
                MySqlCommand myCmd = new MySqlCommand (StrSQL, conn);
                ...
                myCmd.Parameters.AddWithValue ("@tpex001", newKD.no);
                myCmd.Parameters.AddWithValue ("@tpex002", newKD.dt);
                myCmd.Parameters.AddWithValue ("@RSV", newKD.RSV);
                myCmd.Parameters.AddWithValue ("@K", newKD.K);
                myCmd.Parameters.AddWithValue ("@D", newKD.D);
